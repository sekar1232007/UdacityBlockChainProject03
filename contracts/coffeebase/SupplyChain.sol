...
contract SupplyChain {
  ...
  modifier checkValue(uint _upc) {
    ...
    address payable return_wallet = payable(msg.sender);
    return_wallet.transfer(amountToReturn);
    _;
  }
...
// Define a modifier that checks if an item.state of a upc is Processed
  ...
  function packItem(uint _upc) processed(_upc) verifyCaller(items[_upc].originFarmerID) public 
 {
  ...
  function buyItem(uint _upc) forSale(_upc) paidEnough(items[_upc].productPrice) checkValue(_upc) public payable 
    ...
    address payable Farmer_wallet = payable(items[_upc].originFarmerID);
    Farmer_wallet.transfer(items[_upc].productPrice);
    // emit the appropriate event
    emit Sold(_upc);   
  }
